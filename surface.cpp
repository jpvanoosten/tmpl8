...
namespace Tmpl8 {
    ...
    Pixel Blend(Pixel c, float f)
    {
        uint32_t blendFactor = static_cast<uint32_t>(f * 256.0f);
    ...
    void Surface::Plot(float x, float y, Pixel c)
    {
        ...
        float fx = x - static_cast<float>(ix); // Just the fractional part.
        float fy = y - static_cast<float>(iy); // Just the fractional part.

        float w00 = (1.0f - fx) * (1.0f - fy); // Weighted area of the top-left subpixel.
        float w10 = fx * (1.0f - fy);
        float w01 = (1.0f - fx) * fy;
        float w11 = fx * fy;

        assert(std::abs(1.0f - (w00 + w10 + w01 + w11)) < 0.0001f); // Acceptable tolerance.

        Plot(ix, iy, Blend(c, w00));
        Plot(ix + 1, iy, Blend(c, w10));
        Plot(ix, iy + 1, Blend(c, w01));
        Plot(ix + 1, iy + 1, Blend(c, w11));
    ...
    void Surface::Box(float x1, float y1, float x2, float y2, Pixel color)
    {
        Box(
            static_cast<int>(x1),
            static_cast<int>(y1),
    ...
    void Surface::Circle(const ::Circle& circle, Pixel color)
    {
        ...
        for (int i = 0; i < 64; i++)
        {
            ...
            Line(x - r * sinf(r1), y - r * cosf(r1), x - r * sinf(r2), y - r * cosf(r2), color);
        }
    }
    ...
    void Surface::Bar(int x1, int y1, int x2, int y2, Pixel c)
    {
        ...
        x2 = x2 < 0 ? 0 : x2 >= m_Width ? m_Width - 1 : x2;
        y1 = y1 < 0 ? 0 : y1 >= m_Height ? m_Height - 1 : y1;
        y2 = y2 < 0 ? 0 : y2 >= m_Height ? m_Height - 1 : y2;
